// src/pages/DisenarPrendas/ChompaViewer.jsx
...
import { Shirt, Palette, Type, Image, Layers, Sparkles, Save } from "lucide-react";
...
export default function ChompaViewer() {
  ...
  const [productId] = useState("chompa");
  ...
  useEffect(() => {
    setColors(Object.fromEntries(Object.entries(design.zones).map(([k, z]) => [k, z.default])));
    setTextures({});
    setDecals([]);
    setTextDecals([]);
  ...
  const handleGuardarDiseno = async () => {
    ...
    try {
      ...
      for (const a of angles) {
        camera.position.set(Math.sin(a.rotationY) * 5, 1, Math.cos(a.rotationY) * 5);
        camera.lookAt(0, 1, 0);
        gl.render(scene, camera);
      ...
      formData.append("modelo", `CH-${Date.now()}`);
      formData.append("categoria", "chompa");
    ...
    <>
      ...
      <div className="flex flex-col p-4">
        ...
        <div className="flex-1 bg-white rounded-lg shadow-lg relative">
          <PantallaCarga show={loading || loadingLogo} message={loadingLogo ? "Subiendo logo a Cloudinary..." : "Guardando tu diseño 3D..."} />
          <Canvas
            dpr={dpr}
            frameloop="demand"
            gl={{ preserveDrawingBuffer: true, antialias: true, powerPreference: "high-performance" }}
            camera={{ position: product.cameraPosition, fov: product.cameraFov }}
          ...
          <div className="absolute top-2 right-2 bg-black/70 p-3 rounded-lg text-white text-sm">
            ...
            <button
                ...
                className={`mt-4 w-full bg-blue-900 text-white rounded-lg py-2 px-2 font-semibold hover:bg-blue-600 transition-colors ${
