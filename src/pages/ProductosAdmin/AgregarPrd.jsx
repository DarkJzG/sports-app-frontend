...
export default function AgregarProducto() {
  ...
  useEffect(() => {
    ...
    fetch("https://api.cloudinary.com/v1_1/dcn5d4wbo/image/upload", {
      ...
      .then(res => res.json())
      .then(img => setImageUrl(img.secure_url || img.url));
  }, [imagen]);
  ...
  useEffect(() => {
    ...
    // Calcular costo de diseños = cantidad * tarifa guardada en manoObra
    let costoDisenos = 0;
    if (manoObra?.disenos) {
    ...
    const precioMayor = precioVenta * 0.9; // Ej: 10% descuento al mayor
    const ganMayor = precioMayor - total;

    setGananciaMenor(ganMenor);
    setGananciaMayor(ganMayor);
  }, [color, metros, manoObra, cantDisenos, precioVenta]);
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!nombre || !categoria || !tela || !color || !imageUrl) {
      ...
      <form className="max-w-5xl mx-auto flex flex-col lg:flex-row gap-8" onSubmit={handleSubmit}>
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <div className="bg-gray-100 rounded-xl px-6 py-4 shadow mt-3">
            ...
            <p>Tela: ${costos.tela.toFixed(2)}</p>
            <p>Mano de obra: ${costos.manoObra.toFixed(2)}</p>
            <p>Insumos: ${costos.insumos.toFixed(2)}</p>
            <p>Diseños: ${costos.disenos.toFixed(2)}</p>
            <hr className="my-2" />
            <p className="font-bold">Costo Total: ${costos.total.toFixed(2)}</p>
            <p>Ganancia al por menor: ${gananciaMenor.toFixed(2)}</p>
            <p>Ganancia al por mayor (≥ 12): ${gananciaMayor.toFixed(2)}</p>
          </div>

          <button type="submit" className="bg-blue-900 text-white font-bold text-xl px-8 py-4 rounded-xl mt-6 shadow hover:bg-blue-700 w-full">Guardar Producto</button>
