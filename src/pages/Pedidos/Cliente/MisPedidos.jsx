...
const EstadoBadge = ({ estado }) => {
  ...
  const config =
    estadosMap[estado] || { label: estado, color: "bg-gray-100 text-gray-800" };
...
export default function MisPedidos() {
  ...
  const [visibleCount, setVisibleCount] = useState(4); // ← mostrar 4 pedidos recientes
  ...
  const cargarPedidos = async () => {
    try {
      ...
      if (data.ok) {
        ...
        setVisibleCount(4); // siempre empezar mostrando 4
      } else {
  ...
  // Aplicar límite de visibles
  const pedidosVisibles = pedidosFiltrados.slice(0, visibleCount);
  const hayMas = visibleCount < pedidosFiltrados.length;

  // Estadísticas (se mantienen igual, por si las usas luego)
  const estadisticas = {
    ...
    conSaldoPendiente: pedidos.filter(
      (p) => p.infoPago?.saldo_pendiente > 0
    ).length,
  };
        ...
        {pedidos.length > 0 && (
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                En Revisión (
                {pedidos.filter((p) => p.estado === "en_revision").length})
              </button>
              <button
                onClick={() => {
                  setFiltroEstado("en_produccion");
                  setVisibleCount(4);
                }}
                className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
              ...
              >
                En Producción (
                {pedidos.filter((p) => p.estado === "en_produccion").length})
              </button>
              <button
                onClick={() => {
              ...
              >
                Para Retiro (
                {pedidos.filter((p) => p.estado === "retiro").length})
              </button>
              <button
                onClick={() => {
            ...
            <div className="space-y-6">
              {pedidosVisibles.map((pedido) => {
                ...
                const hayPagosPendientes =
                  pedido.pagos?.some((p) => p.estado === "pendiente") || false;
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 p-4 bg-gray-50 rounded-lg">
                      <div>
                        ...
                        <p className="text-lg font-bold text-gray-900">
                          {formatCurrency(
                            infoPago.total_pedido || pedido.costos?.total || 0
                          )}
                        </p>
                      </div>
                      <div>
                    ...
                    {infoPago.porcentaje_pagado !== undefined && (
                      <div className="mb-4">
                        <div className="flex justify-between text-xs text-gray-600 mb-1">
                          ...
                          <span>
                            {Math.round(infoPago.porcentaje_pagado)}%
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full transition-all ${
                              infoPago.porcentaje_pagado >= 100
                                ? "bg-green-600"
                                : infoPago.porcentaje_pagado >= 50
                                ? "bg-blue-600"
                                : "bg-red-600"
                            }`}
                            style={{
                              width: `${Math.min(
                                infoPago.porcentaje_pagado,
                                100
                              )}%`,
                            }}
                          ></div>
                      ...
                      <div className="mb-4 bg-yellow-50 border-l-4 border-yellow-400 p-3 rounded-r-lg">
                        <div className="flex items-center">
                          ...
                          <p className="text-sm text-yellow-800 font-medium">
                            Tienes pagos pendientes de aprobación. No podrás
                            enviar nuevos hasta que sean revisados.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    {saldoPendiente > 0 &&
                      ...
                      pedido.estado !== "cancelado" && (
                        <div className="mb-4">
                          ...
                          >
                            Realizar Pago de{" "}
                            {formatCurrency(saldoPendiente).replace("$", "")}
                          </Link>
                        </div>
                      )}
